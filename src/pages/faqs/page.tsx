...
const faqs = [
  {
    question: 'What purity standards does My Secret Vitality maintain?',
  ...
  },
  {
    ...
    answer: 'Yes. We offer volume discounts and institutional accounts for qualified research laboratories, universities, and pharmaceutical companies. Contact help@mysecretvitality.com with your requirements.',
  },
  {
    question: 'Are these products for human consumption?',
    answer: 'Absolutely not. All products sold by My Secret Vitality are strictly for laboratory research use only. They are not intended for human or animal consumption, diagnosis, treatment, or prevention of any disease.',
  },
];
          ...
          <div className="space-y-3">
            {faqs.map((faq, index) => {
              ...
              return (
                <div
                  key={index}
                  className={`border transition-all duration-500 rounded-[2px] ${
                    isOpen ? 'border-accent-500 bg-background-100/60' : 'border-secondary-500/20 bg-background-100/20'
                  }`}
                >
                  <button
                  ...
                  >
                    <span className="font-heading text-sm tracking-wider uppercase text-foreground-950 pr-4 font-semibold">
                      {faq.question}
                    </span>
                    <span
                      className={`w-6 h-6 flex items-center justify-center text-accent-500 flex-shrink-0 transition-transform duration-300 ${
                        isOpen ? 'rotate-180' : ''
                      }`}
                    >
