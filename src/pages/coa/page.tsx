...
function DetailModal({ entry, onClose }: DetailModalProps) {
  ...
  const openPdf = () => window.open(entry.coaUrl, '_blank', 'noopener,noreferrer');
        ...
        <div className="p-6 md:p-8">
          <div className="flex items-start justify-between mb-6">
            <div>
              ...
              <p className="font-sans text-sm text-[#7A8A76] mt-1 font-light">{entry.peptideCode}</p>
            </div>
            <button
              onClick={onClose}
              className="w-8 h-8 flex items-center justify-center text-[#7A8A76] hover:text-[#2F3430] transition-colors flex-shrink-0"
            >
          ...
          <div className="mb-6">
            ...
            <div className="flex flex-wrap gap-2">
              {entry.methods.map((m) => (
                <span
                  key={m}
                  className="font-mono text-[10px] tracking-wider uppercase px-3 py-1 border border-[#DBD0C2] text-[#7A8A76]"
                >
                  {m}
                </span>
              ))}
        ...
        <div className="max-w-6xl mx-auto px-4 md:px-8">
          ...
          <div className="flex flex-col sm:flex-row gap-3 mb-8">
            <div className="flex-1 flex items-center gap-3 px-4 py-3 bg-white border border-[#DBD0C2]">
              ...
              <input
                ...
                className="flex-1 bg-transparent font-sans text-sm text-[#2F3430] placeholder:text-[#7A8A76]/60 focus:outline-none font-light"
              />
              {searchQuery && (
                <button
                  onClick={() => setSearchQuery('')}
                  className="w-5 h-5 flex items-center justify-center text-[#7A8A76] hover:text-[#2F3430]"
                >
                  <i className="ri-close-line" />
                </button>
              )}
            </div>
            <div className="flex items-center gap-2 px-4 py-3 border border-[#DBD0C2] bg-white text-[#7A8A76] font-mono text-sm">
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-[#DBD0C2] bg-[#E1E4D9]">
                  <th className="text-left font-sans text-[10px] tracking-widest uppercase text-[#7A8A76] px-5 py-3">Product</th>
                  <th className="text-left font-sans text-[10px] tracking-widest uppercase text-[#7A8A76] px-5 py-3">Batch</th>
                  <th className="text-left font-sans text-[10px] tracking-widest uppercase text-[#7A8A76] px-5 py-3">Test Date</th>
                  <th className="text-left font-sans text-[10px] tracking-widest uppercase text-[#7A8A76] px-5 py-3">Purity</th>
                  <th className="text-left font-sans text-[10px] tracking-widest uppercase text-[#7A8A76] px-5 py-3">Lab</th>
                  <th className="text-left font-sans text-[10px] tracking-widest uppercase text-[#7A8A76] px-5 py-3">Status</th>
                  <th className="text-right font-sans text-[10px] tracking-widest uppercase text-[#7A8A76] px-5 py-3">Actions</th>
                </tr>
              </thead>
              <tbody>
                {filtered.map((entry) => (
                  ...
                  >
                    <td className="px-5 py-4">
                      ...
                      <p className="font-sans text-xs text-[#7A8A76] mt-0.5 font-light">{entry.peptideCode}</p>
                    </td>
                    <td className="px-5 py-4">
                      <span className="font-mono text-xs text-[#2F3430]">{entry.batchNumber}</span>
                    </td>
                    <td className="px-5 py-4">
                      <span className="font-mono text-xs text-[#7A8A76]">{entry.testDate}</span>
                    </td>
                    <td className="px-5 py-4">
                      <span className="font-mono text-sm text-[#2F3430] font-semibold">{entry.purity}</span>
                    </td>
                    <td className="px-5 py-4">
                      <span className="font-sans text-xs text-[#7A8A76] font-light">{entry.labName}</span>
                    </td>
                    <td className="px-5 py-4">
                    ...
                    <td className="px-5 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <button
                          ...
                          className="w-8 h-8 flex items-center justify-center text-[#7A8A76] hover:text-[#2F3430] border border-[#DBD0C2] hover:border-[#B08D57] transition-colors"
                          title="Details"
                        >
              ...
              >
                ...
                <div className="flex gap-2">
                  ...
                  <a
                    href={entry.coaUrl}
                    download
                    className="flex items-center justify-center px-3 py-2 border border-[#DBD0C2] text-[#7A8A76] hover:text-[#2F3430] hover:border-[#B08D57] transition-colors"
                    title="Download"
                  >
                    <i className="ri-download-line text-sm" />
                  </a>
          ...
          {filtered.length === 0 && (
            <div className="text-center py-16 border border-[#DBD0C2] bg-white">
              <i className="ri-search-line text-2xl text-[#7A8A76]/40 mb-4 block" />
              <p className="font-sans text-sm text-[#7A8A76]">No records found</p>
              <p className="font-sans text-xs text-[#7A8A76]/60 mt-1">Try a different search term</p>
            </div>
          )}
        </div>
      </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              ...
              <div key={i} className="text-center p-5 border border-[#DBD0C2] bg-white/60">
                ...
                <p className="font-sans text-sm text-[#7A8A76] leading-relaxed font-light">
                  {item.description}
                </p>
              </div>
            ))}
