 ...
 *
 * GET /api/track-order?userId=123
 *   Second mode powering the logged-in Account page (src/pages/account) —
 *   returns that customer's saved profile plus their recent order history.
 *   Folded into this same function (rather than a new api/account.ts) to
 *   stay under Vercel Hobby's 12-function-per-deployment cap — msv/api was
 *   already at 11 deployed functions (14 files minus 3 excluded via
 *   .vercelignore), leaving exactly one slot; this uses it. Mirrors vintage
 *   peps' api/track-order.ts, which has run this same dual-mode pattern
 *   without issue.
 *   Trust model: like the rest of this app's session handling, the WC user
 *   ID lives in sessionStorage and is passed back as a plain query param,
 *   not a signed token — consistent with this endpoint already being fully
 *   public in its ?q= mode. Fine for a low-stakes storefront account view.
 */
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  // ── Account mode: ?userId=123 — profile + order history for a logged-in customer ──
  const userId = String(req.query.userId || '').trim();
  if (userId) {
    if (!/^\d+$/.test(userId)) return res.status(400).json({ error: 'Invalid userId' });
    if (!WC_URL || !hasAuth) return res.status(500).json({ error: 'Server configuration error.' });

    try {
      const [custRes, ordersRes] = await Promise.all([
        fetch(`${WC_URL}/wp-json/wc/v3/customers/${userId}`, { headers: { Authorization: wcAuth() } }),
        fetch(`${WC_URL}/wp-json/wc/v3/orders?customer=${userId}&per_page=25&orderby=date&order=desc`, { headers: { Authorization: wcAuth() } }),
      ]);

      if (!custRes.ok) {
        return res.status(custRes.status === 404 ? 404 : 502).json({ error: 'Could not load account.' });
      }

      const customer = await custRes.json();
      const orders: WcOrder[] = ordersRes.ok ? await ordersRes.json() : [];
      ...
      return res.status(200).json({
        ...
        orders: orders.map((order) => formatPublicOrder(order)),
      });
    } catch (err) {
      console.error('[track-order:account]', err);
      return res.status(500).json({ error: 'Could not load account. Please try again.' });
    }
  }

  // ── Lookup mode: ?q=VTG-... or ?q=<order id> — single public order lookup ──
  const q = String(req.query.q || '').trim();
  if (!q) return res.status(400).json({ error: 'Missing query parameter: q or userId' });
