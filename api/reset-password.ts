 ...
 * Validates the reset key and sets the new password via msv/v1/reset-password.
 */
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    const r = await fetch(`${WC_URL}/wp-json/msv/v1/reset-password`, {
      method: 'POST',
      headers: {
    ...
    console.error('[msv/reset-password]', e);
