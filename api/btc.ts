...
export default async function handler(req: VercelRequest, res: VercelResponse): Promise<void> {
  ...
  // Supports xpub (BIP44/49), ypub (BIP49), zpub (BIP84 — native SegWit bc1q...)
  let address: string

  try {
    const stripped = zpub.replace(/\s+/g, '')
    const keyMatch = stripped.match(/[zxyZXY]pub[1-9A-HJ-NP-Za-km-z]{100,115}/)
    if (!keyMatch) {
      throw new Error(`Invalid extended public key format (got: ${stripped.slice(0, 20)}…)`)
    }
    const cleanKey = keyMatch[0]

    // Map version bytes so @scure/bip32 can parse non-standard zpub/ypub prefixes
    ...
    if (cleanKey.startsWith('zpub')) {
      ...
      versions = { public: 0x0488b21e, private: 0x0488ade4 } // xpub
    }

    const root = HDKey.fromExtendedKey(cleanKey, versions)
  ...
  try {
    const hookUrl = `https://${VERCEL_URL}/api/btc`
