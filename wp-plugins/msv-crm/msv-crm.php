<?php
/**
 * Plugin Name:  My Secret Vitality — CRM
...
define( 'MSV_CRM_VERSION',  '1.0.0' );
define( 'MSV_CRM_TABLE',    'msv_crm_contacts' );
define( 'MSV_CRM_OPT_FROM_NAME',  'msv_crm_from_name' );
define( 'MSV_CRM_OPT_FROM_EMAIL', 'msv_crm_from_email' );
define( 'MSV_CRM_OPT_DOUBLE_OI',  'msv_crm_double_optin' );
define( 'MSV_CRM_OPT_HMAC_KEY',   'msv_crm_hmac_key' );
define( 'MSV_CRM_PER_PAGE',       50 );

// ─── Activation — create DB table ────────────────────────────────────────────

register_activation_hook( __FILE__, 'msv_crm_activate' );
function msv_crm_activate() {
    global $wpdb;
    $table   = $wpdb->prefix . MSV_CRM_TABLE;
    ...
    if ( ! get_option( MSV_CRM_OPT_HMAC_KEY ) ) {
        update_option( MSV_CRM_OPT_HMAC_KEY, bin2hex( random_bytes( 32 ) ) );
    }
}

// ─── Helpers ──────────────────────────────────────────────────────────────────

function msv_crm_table(): string {
    global $wpdb;
    return $wpdb->prefix . MSV_CRM_TABLE;
}

function msv_crm_from_name(): string {
    return get_option( MSV_CRM_OPT_FROM_NAME, 'My Secret Vitality' );
}

function msv_crm_from_email(): string {
    return get_option( MSV_CRM_OPT_FROM_EMAIL, 'no-reply@mysecretvitality.com' );
}

function msv_crm_unsubscribe_token( string $email ): string {
    $key = get_option( MSV_CRM_OPT_HMAC_KEY, '' );
    return hash_hmac( 'sha256', strtolower( $email ), $key );
}

function msv_crm_unsubscribe_url( string $email ): string {
    return add_query_arg( [
        'token' => msv_crm_unsubscribe_token( $email ),
        'email' => rawurlencode( $email ),
    ], rest_url( 'msv-crm/v1/unsubscribe' ) );
}

function msv_crm_type_label( string $type ): string {
    return match ( $type ) {
...
function msv_crm_status_badge( string $status ): string {
    $map = [
        'active'       => '#22c55e',
        'pending'      => '#f59e0b',
        'unsubscribed' => '#6b7280',
...
add_action( 'admin_menu', 'msv_crm_admin_menu' );
function msv_crm_admin_menu() {
    add_menu_page(
        'MSV CRM',
        'MSV CRM',
        'manage_woocommerce',
        'msv-crm',
        'msv_crm_page_dashboard',
        'dashicons-email-alt',
        56
    );
    add_submenu_page( 'msv-crm', 'Dashboard',   'Dashboard',   'manage_woocommerce', 'msv-crm',            'msv_crm_page_dashboard' );
    add_submenu_page( 'msv-crm', 'Contacts',    'Contacts',    'manage_woocommerce', 'msv-crm-contacts',   'msv_crm_page_contacts' );
    add_submenu_page( 'msv-crm', 'Newsletter',  'Newsletter',  'manage_woocommerce', 'msv-crm-newsletter', 'msv_crm_page_newsletter' );
    add_submenu_page( 'msv-crm', 'Settings',    'Settings',    'manage_woocommerce', 'msv-crm-settings',   'msv_crm_page_settings' );
}

add_action( 'admin_init', 'msv_crm_register_settings' );
function msv_crm_register_settings() {
    foreach ( [ MSV_CRM_OPT_FROM_NAME, MSV_CRM_OPT_FROM_EMAIL, MSV_CRM_OPT_DOUBLE_OI ] as $opt ) {
        register_setting( 'msv_crm_settings', $opt, [ 'sanitize_callback' => 'sanitize_text_field' ] );
    }
}

// ─── Admin: Dashboard ─────────────────────────────────────────────────────────

function msv_crm_page_dashboard() {
    global $wpdb;
    $t = msv_crm_table();
    ...
    <table class="widefat striped">
        ...
        <?php foreach ( $recent as $row ) : ?>
            <tr>
                ...
                <td><?php echo msv_crm_type_label( $row['type'] ); ?></td>
                <td><code><?php echo esc_html( $row['source'] ); ?></code></td>
                <td><?php echo msv_crm_status_badge( $row['status'] ); ?></td>
...
function msv_crm_page_contacts() {
    global $wpdb;
    $t = msv_crm_table();

    // Handle delete
    if ( isset( $_GET['delete_id'] ) && check_admin_referer( 'msv_crm_delete_' . (int) $_GET['delete_id'] ) ) {
    ...
    $offset = ( $paged - 1 ) * MSV_CRM_PER_PAGE;
    ...
    $rows      = $wpdb->get_results( "SELECT * {$sql_base} ORDER BY created_at DESC LIMIT " . MSV_CRM_PER_PAGE . " OFFSET {$offset}", ARRAY_A );
    $pages     = ceil( $total / MSV_CRM_PER_PAGE );

    $base_url  = admin_url( 'admin.php?page=msv-crm-contacts' );

    // CSV export
    if ( isset( $_GET['export'] ) && current_user_can( 'manage_woocommerce' ) ) {
        msv_crm_export_csv( $sql_base );
        exit;
    }
    ?>
    <div class="wrap">
    ...
    <form method="get" style="margin:16px 0;display:flex;gap:8px;flex-wrap:wrap;align-items:center;">
        <input type="hidden" name="page" value="msv-crm-contacts" />
        <select name="type" onchange="this.form.submit()">
            <option value="">All Types</option>
            <?php foreach ( [ 'newsletter', 'contact', 'order' ] as $opt ) : ?>
            <option value="<?php echo $opt; ?>" <?php selected( $type, $opt ); ?>><?php echo msv_crm_type_label( $opt ); ?></option>
            <?php endforeach; ?>
        </select>
            ...
            <tr>
                ...
                <td>
                    ...
                    $del_url = wp_nonce_url(
                        add_query_arg( [ 'page' => 'msv-crm-contacts', 'delete_id' => $row['id'] ], admin_url( 'admin.php' ) ),
                        'msv_crm_delete_' . $row['id']
                    );
                    ?>
...
function msv_crm_page_newsletter() {
    global $wpdb;
    $t = msv_crm_table();
    ...
    if ( isset( $_POST['msv_crm_broadcast_nonce'] ) && wp_verify_nonce( $_POST['msv_crm_broadcast_nonce'], 'msv_crm_broadcast' ) ) {
        ...
        if ( $subject && $body ) {
            ...
            foreach ( $subscribers as $sub ) {
                $unsub_link = msv_crm_unsubscribe_url( $sub['email'] );
                $html = msv_crm_newsletter_template( $sub['name'], $subject, $body, $unsub_link );
                $headers = [
                    'Content-Type: text/html; charset=UTF-8',
                    'From: ' . msv_crm_from_name() . ' <' . msv_crm_from_email() . '>',
                ];
    ...
    <div style="display:grid;grid-template-columns:2fr 1fr;gap:24px;margin-top:20px;align-items:start;">
        ...
        <div style="background:#fff;border:1px solid #e5e7eb;border-radius:8px;padding:24px;">
            ...
            <form method="post">
                <?php wp_nonce_field( 'msv_crm_broadcast', 'msv_crm_broadcast_nonce' ); ?>
            ...
            <div style="background:#fff;border:1px solid #e5e7eb;border-radius:8px;padding:20px;">
                ...
                foreach ( $by_status as $row ) {
                    echo '<div style="display:flex;justify-content:space-between;padding:6px 0;border-bottom:1px solid #f3f4f6;">'
                        . '<span>' . msv_crm_status_badge( $row['status'] ) . '</span>'
                        . '<strong>' . number_format( $row['cnt'] ) . '</strong></div>';
                }
                ?>
                <div style="margin-top:12px;">
                    <a href="<?php echo esc_url( add_query_arg( [ 'page' => 'msv-crm-contacts', 'type' => 'newsletter' ], admin_url( 'admin.php' ) ) ); ?>"
...
function msv_crm_page_settings() {
    if ( isset( $_GET['updated'] ) ) {
        echo '<div class="notice notice-success"><p>Settings saved.</p></div>';
    }
    $double_optin = get_option( MSV_CRM_OPT_DOUBLE_OI, '1' );
    ?>
    <div class="wrap">
    <h1>CRM Settings</h1>
    ...
    <form method="post" action="options.php">
        <?php settings_fields( 'msv_crm_settings' ); ?>
        <table class="form-table">
            <tr>
                <th><label for="<?php echo MSV_CRM_OPT_FROM_NAME; ?>">From Name</label></th>
                <td>
                    <input type="text" id="<?php echo MSV_CRM_OPT_FROM_NAME; ?>"
                           name="<?php echo MSV_CRM_OPT_FROM_NAME; ?>"
                           value="<?php echo esc_attr( msv_crm_from_name() ); ?>" class="regular-text" />
                </td>
            </tr>
            <tr>
                <th><label for="<?php echo MSV_CRM_OPT_FROM_EMAIL; ?>">From Email</label></th>
                <td>
                    <input type="email" id="<?php echo MSV_CRM_OPT_FROM_EMAIL; ?>"
                           name="<?php echo MSV_CRM_OPT_FROM_EMAIL; ?>"
                           value="<?php echo esc_attr( msv_crm_from_email() ); ?>" class="regular-text" />
            ...
            <tr>
                ...
                <td>
                    <label>
                        <input type="checkbox" name="<?php echo MSV_CRM_OPT_DOUBLE_OI; ?>" value="1"
    ...
    <form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
        <input type="hidden" name="action" value="msv_crm_test_email" />
        <?php wp_nonce_field( 'msv_crm_test_email' ); ?>
        <table class="form-table">
            <tr>
...
add_action( 'admin_post_msv_crm_test_email', 'msv_crm_handle_test_email' );
function msv_crm_handle_test_email() {
    check_admin_referer( 'msv_crm_test_email' );
    if ( ! current_user_can( 'manage_woocommerce' ) ) wp_die( 'Unauthorized' );

    $to      = sanitize_email( $_POST['test_email'] ?? '' );
    $subject = '[MSV CRM] Test email from ' . get_bloginfo( 'name' );
    $body    = msv_crm_newsletter_template(
        'Admin',
        $subject,
        '<p>This is a test email sent from MSV CRM. If you received this, your SMTP is configured correctly.</p>',
        '#'
    );
    $headers = [
        'Content-Type: text/html; charset=UTF-8',
    ...
    wp_redirect( add_query_arg( [ 'page' => 'msv-crm-settings', 'updated' => $ok ? '1' : 'fail' ], admin_url( 'admin.php' ) ) );
    exit;
}

// ─── CSV Export ───────────────────────────────────────────────────────────────

function msv_crm_export_csv( string $sql_base ) {
    global $wpdb;
    $rows = $wpdb->get_results( "SELECT * {$sql_base} ORDER BY created_at DESC", ARRAY_A );

    header( 'Content-Type: text/csv; charset=utf-8' );
    header( 'Content-Disposition: attachment; filename="msv-crm-export-' . date( 'Y-m-d' ) . '.csv"' );
...
function msv_crm_newsletter_template( string $name, string $subject, string $body, string $unsub_url ): string {
...
function msv_crm_confirmation_template( string $name, string $confirm_url ): string {
    $site = get_bloginfo( 'name' );
    $greeting = $name ? "Hi {$name}," : 'Hello,';
    return <<<HTML
<!DOCTYPE html>
...
add_action( 'rest_api_init', 'msv_crm_register_routes' );
function msv_crm_register_routes() {

    // Public: newsletter subscribe
    register_rest_route( 'msv-crm/v1', '/subscribe', [
        'methods'             => 'POST',
        'callback'            => 'msv_crm_rest_subscribe',
        'permission_callback' => '__return_true',
    ] );

    // Public: email confirm (double opt-in click)
    register_rest_route( 'msv-crm/v1', '/confirm', [
        'methods'             => 'GET',
        'callback'            => 'msv_crm_rest_confirm',
        'permission_callback' => '__return_true',
    ] );

    // Public: unsubscribe (link in emails)
    register_rest_route( 'msv-crm/v1', '/unsubscribe', [
        'methods'             => 'GET',
        'callback'            => 'msv_crm_rest_unsubscribe',
        'permission_callback' => '__return_true',
    ] );

    // Public: contact form
    register_rest_route( 'msv-crm/v1', '/contact', [
        'methods'             => 'POST',
        'callback'            => 'msv_crm_rest_contact',
        'permission_callback' => '__return_true',
    ] );

    // Admin: list contacts
    register_rest_route( 'msv-crm/v1', '/contacts', [
        'methods'             => 'GET',
        'callback'            => 'msv_crm_rest_list_contacts',
        'permission_callback' => fn() => current_user_can( 'manage_woocommerce' ),
    ] );

    // Admin: delete contact
    register_rest_route( 'msv-crm/v1', '/contacts/(?P<id>\d+)', [
        'methods'             => 'DELETE',
        'callback'            => 'msv_crm_rest_delete_contact',
        'permission_callback' => fn() => current_user_can( 'manage_woocommerce' ),
    ] );
}
...
function msv_crm_rest_subscribe( WP_REST_Request $request ): WP_REST_Response {
    global $wpdb;
    $t = msv_crm_table();
    ...
    if ( $existing ) {
        ...
        if ( get_option( MSV_CRM_OPT_DOUBLE_OI, '1' ) === '1' ) {
            msv_crm_send_confirmation( $email, $name );
        }
        return new WP_REST_Response( [ 'ok' => true, 'message' => 'confirmation_resent' ], 200 );
    }

    $double_optin = get_option( MSV_CRM_OPT_DOUBLE_OI, '1' ) === '1';
...
function msv_crm_send_confirmation( string $email, string $name ) {
    $token = msv_crm_unsubscribe_token( $email . '|confirm' );
    $confirm_url = add_query_arg( [
        'token' => $token,
        'email' => rawurlencode( $email ),
    ], rest_url( 'msv-crm/v1/confirm' ) );

    $subject = 'Confirm your subscription — ' . get_bloginfo( 'name' );
    $body    = msv_crm_confirmation_template( $name, $confirm_url );
    $headers = [
        'Content-Type: text/html; charset=UTF-8',
        'From: ' . msv_crm_from_name() . ' <' . msv_crm_from_email() . '>',
    ];
...
function msv_crm_rest_confirm( WP_REST_Request $request ): WP_REST_Response {
    global $wpdb;
    $t = msv_crm_table();
    ...
    $expected = msv_crm_unsubscribe_token( $email . '|confirm' );
...
function msv_crm_rest_unsubscribe( WP_REST_Request $request ): WP_REST_Response {
    global $wpdb;
    $t = msv_crm_table();
    ...
    if ( ! hash_equals( msv_crm_unsubscribe_token( $email ), $token ) ) {
...
function msv_crm_rest_contact( WP_REST_Request $request ): WP_REST_Response {
    global $wpdb;
    $t = msv_crm_table();
    ...
    wp_mail( msv_crm_from_email(), $admin_subject, $admin_body, $headers );
    ...
    wp_mail( $email, 'We received your message — ' . get_bloginfo( 'name' ), msv_crm_newsletter_template( $name, 'Message received', $auto_reply, '#' ), $headers );
...
function msv_crm_rest_list_contacts( WP_REST_Request $request ): WP_REST_Response {
    global $wpdb;
    $t = msv_crm_table();
...
function msv_crm_rest_delete_contact( WP_REST_Request $request ): WP_REST_Response {
    global $wpdb;
    $id = (int) $request->get_param( 'id' );
    $deleted = $wpdb->delete( msv_crm_table(), [ 'id' => $id ], [ '%d' ] );
    return $deleted
...
add_action( 'woocommerce_checkout_order_processed', 'msv_crm_capture_order_email', 10, 3 );
function msv_crm_capture_order_email( $order_id, $posted_data, $order ) {
    global $wpdb;
    $t = msv_crm_table();
