...
define( 'MSVROUTER_VERSION', '1.1.0' );
define( 'MSVROUTER_PLUGIN_DIR', plugin_dir_path( __FILE__ ) );
define( 'MSVROUTER_PLUGIN_URL', plugin_dir_url( __FILE__ ) );

// ── Where to look for index.html (in order) ───────────────────────────────────
// 1. plugin folder:  wp-content/plugins/msv-router/app/index.html  ← RECOMMENDED
// 2. uploads folder: wp-content/uploads/msv-app/index.html
// 3. wp-config.php override: define('MSVROUTER_APP_DIR', '/absolute/path');
//                             define('MSVROUTER_APP_URL', 'https://site.com/path');

function msvrouter_find_app(): array {
    $candidates = [];

    // 1. Plugin's own app/ subfolder — always resolves correctly
    $candidates[] = [
        'dir' => rtrim( MSVROUTER_PLUGIN_DIR, '/' ) . '/app',
        'url' => rtrim( MSVROUTER_PLUGIN_URL, '/' ) . '/app',
        'label' => 'Plugin folder (wp-content/plugins/msv-router/app/)',
    ];

    // 2. wp-content/uploads/msv-app/
    $candidates[] = [
        'dir' => rtrim( WP_CONTENT_DIR, '/' ) . '/uploads/msv-app',
        'url' => rtrim( WP_CONTENT_URL, '/' ) . '/uploads/msv-app',
        'label' => 'Uploads folder (wp-content/uploads/msv-app/)',
    ];

    // 3. wp-config.php override
    if ( defined('MSVROUTER_APP_DIR') && defined('MSVROUTER_APP_URL') ) {
        $candidates[] = [
            'dir'   => MSVROUTER_APP_DIR,
            'url'   => MSVROUTER_APP_URL,
 ...
 */
function msvrouter_get_head_inject(): string {
    $tags = [];
    ...
    $ga4_id = defined('MSVROUTER_GA4_ID') ? MSVROUTER_GA4_ID : '';  // set in wp-config.php: define('MSVROUTER_GA4_ID','G-XXXXXXXXXX');
    if ( $ga4_id ) {
...
HTML;
    ...
    // ── Google Tag Manager ────────────────────────────────────────────────
    // Alternative to GA4 direct. Set in wp-config.php: define('MSVROUTER_GTM_ID','GTM-XXXXXXX');
    $gtm_id = defined('MSVROUTER_GTM_ID') ? MSVROUTER_GTM_ID : '';
    if ( $gtm_id ) {
...
class MSVROUTER_Router {
    ...
    private static function serve_index( string $index_path, string $app_url ) {
        ...
        // both mysecretvitality.com AND www.mysecretvitality.com without CORS errors.
        ...
        $inject = msvrouter_get_head_inject();
        if ( $inject ) {
    ...
    private static function render_not_deployed( array $candidates ) {
        ...
        <title>MSV — App Not Deployed</title>
        <style>
        ...
        <body><div class="box">
          ...
          <div class="hint">
            ...
            <code>wp-content/plugins/msv-router/app/</code>
          </div>
        </div></body></html>';
    }
}
...
class MSVROUTER_Admin {
    ...
    public static function render_page() {
        ...
        $plugin_app  = rtrim( MSVROUTER_PLUGIN_DIR, '/' ) . '/app';
        $plugin_url  = rtrim( MSVROUTER_PLUGIN_URL, '/' ) . '/app';
        ...
        $import_results = null;
        if (
            isset( $_POST['msvrouter_import_tabs'] ) &&
            check_admin_referer( 'msvrouter_import_tabs' )
        ) {
            $import_results = msvrouter_run_tab_import();
        }

        // Handle bulk product create action
        $bulk_create_results = null;
        if (
            isset( $_POST['msvrouter_bulk_create'] ) &&
            check_admin_referer( 'msvrouter_bulk_create' )
        ) {
            $bulk_create_results = msvrouter_bulk_create_products();
        }
        ?>
        <div class="wrap">
            <h1>🛡 MSV Frontend Router <span style="font-size:13px;font-weight:400;color:#888;">v<?php echo MSVROUTER_VERSION; ?></span></h1>
            ...
            <div style="background:#fff;border:1px solid #e0e0e0;padding:24px;max-width:640px;margin:20px 0 0;border-radius:4px;">
                ...
                <form method="post">
                    <?php wp_nonce_field( 'msvrouter_import_tabs' ); ?>
                    <input type="hidden" name="msvrouter_import_tabs" value="1">
            ...
            <div style="background:#fff;border:1px solid #e0e0e0;padding:24px;max-width:640px;margin:20px 0 0;border-radius:4px;">
                <h2 style="margin-top:0;">🆕 Bulk Create All MSV Products</h2>
                <p style="font-size:13px;color:#555;margin:0 0 6px;">
                    Creates the following WooCommerce products (skips any that already exist by SKU or slug):
                </p>
                <p style="font-size:12px;font-weight:700;color:#444;margin:6px 0 4px;">May 2026 Batch (6 products):</p>
                <ul style="font-size:12px;color:#444;margin:0 0 8px;line-height:1.9;">
                    <li>Thymosin Alpha-1 — 10mg — <strong>$85</strong></li>
                    <li>Hexarelin — 10mg — <strong>$120</strong></li>
                    <li>Oxytocin — 5mg — <strong>$60</strong></li>
                    <li>Kisspeptin-10 — 10mg — <strong>$70</strong></li>
                    <li>AOD-9604 — 5mg — <strong>$65</strong></li>
                    <li>Melanotan II — 10mg — <strong>$50</strong> <em>(renames draft Melanotan I if found)</em></li>
                </ul>
                <p style="font-size:12px;font-weight:700;color:#444;margin:6px 0 4px;">June 2026 Batch — Jerod's Product List (10 products):</p>
                <ul style="font-size:12px;color:#444;margin:0 0 16px;line-height:1.9;">
                    <li>BPC-157/TB-500 — 10mg — <strong>$85</strong></li>
                    <li>Semaglutide — 5mg — <strong>$120</strong></li>
                    <li>Tirzepatide — 10mg — <strong>$165</strong></li>
                    <li>Tirzepatide — 15mg — <strong>$225</strong></li>
                    <li>Retatrutide — 15mg — <strong>$245</strong></li>
                    <li>KPV — 10mg — <strong>$75</strong></li>
                    <li>GHK-Cu — 50mg — <strong>$95</strong></li>
                    <li>Tesamorelin — 20mg — <strong>$255</strong></li>
                    <li>Thymosin Alpha-1 — 5mg — <strong>$105</strong></li>
                    <li>Cartalax — 20mg — <strong>$110</strong></li>
                </ul>
                <p style="font-size:12px;color:#888;margin:0 0 16px;">
                    After running, copy the returned product IDs back into <code>msvrouter_get_tab_data()</code> (placeholder IDs 6–15) to enable the tab import for these products.
                </p>
                ...
                <?php if ( $bulk_create_results !== null ) : ?>
                    <div style="background:#f0fdf4;border:1px solid #bbf7d0;padding:16px;border-radius:4px;margin-bottom:16px;">
                        ...
                        <p style="font-size:11px;color:#555;margin:12px 0 0;">
                            ☝️ Copy the product IDs above into <code>msvrouter_get_tab_data()</code> then run Product Tabs Import.
                        </p>
                    </div>
                <?php endif; ?>

                <form method="post">
                    <?php wp_nonce_field( 'msvrouter_bulk_create' ); ?>
                    <input type="hidden" name="msvrouter_bulk_create" value="1">
                    <button type="submit" class="button button-primary" style="font-size:14px;height:38px;padding:0 20px;background:#0ea5e9;border-color:#0284c7;">
                        🚀 Create All 16 MSV Products in WooCommerce
                    </button>
                </form>
            </div>
            <div style="background:#fff;border:1px solid #e0e0e0;padding:24px;max-width:640px;margin:20px 0 0;border-radius:4px;">
            ...
            <div style="background:#f0fdf4;border:1px solid #bbf7d0;padding:24px;max-width:640px;margin:16px 0 0;border-radius:4px;">
                ...
                <ol style="font-size:13px;color:#444;line-height:2.2;">
                    <li>Build: <code>BASE_PATH=/wp-content/plugins/msv-router/app/ npm run build</code></li>
                    <li>In GoDaddy cPanel File Manager go to:<br>
                        <code>public_html/wp-content/plugins/msv-router/</code></li>
            ...
            <div style="background:#f8f7f5;border:1px solid #e0e0e0;padding:24px;max-width:640px;margin:16px 0 0;border-radius:4px;">
                ...
                <ol style="font-size:13px;color:#444;line-height:2.2;">
                    <li>Build: <code>BASE_PATH=/wp-content/uploads/msv-app/ npm run build</code></li>
                    <li>Go to: <code>public_html/wp-content/uploads/</code></li>
                    <li>Create folder <code>msv-app</code>, upload <code>index.html</code> + <code>assets/</code></li>
                </ol>
            </div>
            ...
            <div style="background:#f8f7f5;border:1px solid #e0e0e0;padding:24px;max-width:640px;margin:16px 0 0;border-radius:4px;">
                ...
                <ul style="font-size:13px;color:#444;line-height:2;">
                    ...
                    <li><code>/wp-json/*</code> — REST API (WooCommerce + MSV CMS)</li>
    ...
    public static function show_notices() {
        ...
        $app = msvrouter_find_app();
        if ( empty( $app['found'] ) && strpos( $screen->id, 'msv-router' ) !== false ) {
            $plugin_path = rtrim( MSVROUTER_PLUGIN_DIR, '/' ) . '/app/index.html';
            echo '<div class="notice notice-error"><p>
                <strong>MSV Router:</strong> React app not deployed.
...
/**
 * Expose _msv_coa_images and _msv_additional_info in the
...
add_filter( 'woocommerce_rest_prepare_product_object', function( $response, $object ) {
    $id   = $object->get_id();
    $coa  = get_post_meta( $id, '_msv_coa_images',      true );
    $info = get_post_meta( $id, '_msv_additional_info', true );
    $purity_pdf    = get_post_meta( $id, '_msv_coa_purity_pdf',    true );
    $endotoxin_pdf = get_post_meta( $id, '_msv_coa_endotoxin_pdf', true );

    $data = $response->get_data();
    $meta = $data['meta_data'] ?? [];

    if ( $coa )           $meta[] = [ 'key' => '_msv_coa_images',         'value' => $coa ];
    if ( $info )          $meta[] = [ 'key' => '_msv_additional_info',    'value' => $info ];
    if ( $purity_pdf )    $meta[] = [ 'key' => '_msv_coa_purity_pdf',     'value' => $purity_pdf ];
    if ( $endotoxin_pdf ) $meta[] = [ 'key' => '_msv_coa_endotoxin_pdf',  'value' => $endotoxin_pdf ];
 ...
 */
function msvrouter_get_tab_data(): array {
    return [
        133  => [ 'name' => 'TESAMORELIN – 10MG', 'coa' => [ 'https://mysecretvitality.com/wp-content/uploads/2024/09/1773253842237-3b359788-7187-42c5-a47b-b9efd3fab57b_1.jpg', 'https://mysecretvitality.com/wp-content/uploads/2024/09/1773253842237-3b359788-7187-42c5-a47b-b9efd3fab57b_2.jpg' ], 'info' => 'Tesamorelin — GHRH Analogue for Endocrine & Metabolic Research. Tesamorelin is a synthetic growth hormone–releasing hormone (GHRH) analogue widely examined in laboratory research for its role in growth hormone (GH) axis signaling and systemic metabolic regulation. My Secret Vitality supplies Tesamorelin as a lyophilized research-grade compound. Research areas: Mechanisms governing GH-axis stimulation and downstream IGF-1 signaling | Metabolic regulation involving lipid mobilization and energy-balance pathways | Endocrine-driven changes in tissue composition | Interactions between GH signaling and neuronal or cognitive pathways. Format: Lyophilized powder. For laboratory research use only.' ],
        124  => [ 'name' => 'GLP-3 (RT) – 30MG', 'coa' => [ 'https://mysecretvitality.com/wp-content/uploads/2024/09/VP-4892263-Retatrutide-Purity_page-0001.jpg', 'https://mysecretvitality.com/wp-content/uploads/2024/09/VP-4892263-Retatrutide-Purity_page-0002.jpg' ], 'info' => 'GLP-3 (RT) — Multi-Receptor Research Peptide. GLP-3 (RT) (also referenced as GLP3-RTA or LY-3437943) is a synthetic research peptide evaluated in advanced metabolic and endocrine signaling models. Characterised by interaction with GLP-1, GIP, and glucagon receptor pathways. Provided as lyophilized powder by My Secret Vitality. For laboratory research use only.' ],
        128  => [ 'name' => 'NAD+ – 500MG', 'coa' => [ 'https://mysecretvitality.com/wp-content/uploads/2024/09/VP-6801827-NAD-Purity-images-0-scaled.jpg', 'https://mysecretvitality.com/wp-content/uploads/2024/09/VP-6801827-NAD-Purity-images-1-scaled.jpg' ], 'info' => 'NAD+ (Nicotinamide Adenine Dinucleotide) — Cellular Energy & Redox Research Compound (500 mg). NAD+ is a fundamental coenzyme widely examined for its central role in cellular energy metabolism and oxidation-reduction (redox) signaling. Research areas: Electron-transfer reactions essential to metabolic flux | Regulation of oxidative and reductive states within cells | Support of enzymatic pathways tied to cellular repair and adaptation | Mechanisms underlying metabolic resilience and energy-production efficiency. Format: 500 mg lyophilized powder. For laboratory research use only.' ],
        131  => [ 'name' => 'GHK-Cu – 100MG', 'coa' => [ 'https://mysecretvitality.com/wp-content/uploads/2024/09/VP-4215641-GHK-Cu-Purity-images-0-scaled.jpg', 'https://mysecretvitality.com/wp-content/uploads/2024/09/VP-4215641-GHK-Cu-Purity-images-1-scaled.jpg' ], 'info' => 'GHK-Cu — Copper Tripeptide Research Compound. GHK-Cu is a copper-binding tripeptide complex widely examined in laboratory research for its involvement in extracellular matrix (ECM) signaling, cellular repair pathways, and redox-related biological processes. Research areas: Gene-expression patterns related to tissue repair and remodeling | Cellular migration and structural adaptation within connective-tissue models | Mechanisms underlying inflammation resolution and antioxidant defense | Cutaneous regeneration pathways and ECM-driven cellular communication. Format: Lyophilized powder. For laboratory research use only.' ],
        135  => [ 'name' => 'GLOW – 70MG', 'coa' => [ 'https://mysecretvitality.com/wp-content/uploads/2024/09/VP-1143805-GLOW-Purity_page-0001.jpg', 'https://mysecretvitality.com/wp-content/uploads/2024/09/VP-1143805-GLOW-Purity_page-0002.jpg', 'https://mysecretvitality.com/wp-content/uploads/2024/09/VP-1143805-GLOW-Purity_page-0003.jpg', 'https://mysecretvitality.com/wp-content/uploads/2024/09/VP-1143805-GLOW-Purity_page-0004.jpg', 'https://mysecretvitality.com/wp-content/uploads/2024/09/VP-1143805-GLOW-Purity_page-0005.jpg', 'https://mysecretvitality.com/wp-content/uploads/2024/09/VP-1143805-GLOW-Purity_page-0006.jpg' ], 'info' => 'GLOW — Multi-Component Research Peptide Blend (GHK-Cu 50mg + TB-500 10mg + BPC-157 10mg). Total 70mg per vial in lyophilized form. Components: GHK-Cu (50mg) — fibroblast activation, ECM regulation, antioxidant activity, gene-expression patterns linked to tissue remodeling | TB-500 (10mg) — cellular migration, structural recovery, cytoskeletal organization within soft-tissue models | BPC-157 (10mg) — microvascular signaling, inflammatory modulation, gastrointestinal or connective-tissue repair pathways. Format: Lyophilized powder. For laboratory research use only.' ],
        ...
        5907 => [ 'name' => 'KLOW – 80MG', 'coa' => [ 'https://mysecretvitality.com/wp-content/uploads/2026/04/VP-9089847-KLOW-Purity_page-0001.jpg', 'https://mysecretvitality.com/wp-content/uploads/2026/04/VP-9089847-KLOW-Purity_page-0002.jpg', 'https://mysecretvitality.com/wp-content/uploads/2026/04/VP-9089847-KLOW-Purity_page-0003.jpg', 'https://mysecretvitality.com/wp-content/uploads/2026/04/VP-9089847-KLOW-Purity_page-0004.jpg', 'https://mysecretvitality.com/wp-content/uploads/2026/04/VP-9089847-KLOW-Purity_page-0005.jpg', 'https://mysecretvitality.com/wp-content/uploads/2026/04/VP-9089847-KLOW-Purity_page-0006.jpg', 'https://mysecretvitality.com/wp-content/uploads/2026/04/VP-9089847-KLOW-Purity_page-0007.jpg', 'https://mysecretvitality.com/wp-content/uploads/2026/04/VP-9089847-KLOW-Purity_page-0008.jpg' ], 'info' => 'KLOW — Multi-Peptide Research Blend (80mg). Blend: BPC-157 + GHK-Cu + TB-500 + KPV. Components: BPC-157 — extracellular-matrix dynamics, cell-migration pathways, gastrointestinal-associated research frameworks | GHK-Cu — copper-binding interactions, ECM-related signaling, oxidative-stress response pathways | TB-500 — actin-binding proteins, cytoskeletal organization, cell-motility research models | KPV — inflammatory-signaling pathways, immune-cell response profiles, peptide-mediated regulatory mechanisms. Format: Lyophilized powder. Storage: -20°C. For laboratory research use only.' ],
        ...
        // after running the "Bulk Create All MSV Products" button in the admin panel.
        // Use the returned product IDs to update these entries, then run Product Tabs Import.
        //
        // Existing 6 (May 2026 batch):
        //   thymosin-alpha-1-10mg | hexarelin-10mg | oxytocin-5mg |
        //   kisspeptin-10-10mg | aod-9604-5mg | melanotan-2-10mg
        //
        // New Jerod batch (June 2026):
        //   bpc-157-tb-500-10mg | semaglutide-5mg | tirzepatide-10mg |
        //   tirzepatide-15mg | retatrutide-15mg | kpv-10mg | ghk-cu-50mg |
        //   tesamorelin-20mg | thymosin-alpha-1-5mg | cartalax-20mg
        ...
        // ── JEROD JUNE 2026 BATCH — placeholders (IDs 6–15) ──────────────────
        // Replace these IDs with the real WooCommerce product IDs after running
        // "Bulk Create All MSV Products" in the admin panel.
        6  => [ 'name' => 'BPC-157/TB-500 – 10MG',
            'coa'  => [],
            'info' => 'BPC-157/TB-500 — Dual-Peptide Research Blend (10mg). BPC-157 (5mg) + TB-500 (5mg). BPC-157: gastric-derived pentadecapeptide studied for angiogenic and microvascular signaling, collagen organization, inflammatory-pathway modulation. TB-500: synthetic fragment of thymosin beta-4, examined for actin-mediated cell migration, cytoskeletal organization, ECM remodeling. Format: Lyophilized powder. For laboratory research use only.',
        ],
        7  => [ 'name' => 'SEMAGLUTIDE – 5MG',
            'coa'  => [],
            'info' => 'Semaglutide — GLP-1 Receptor Agonist Research Peptide (5mg). Synthetic GLP-1 receptor agonist. High GLP-1R affinity | Resistance to DPP-4 degradation | Extended stability via C-18 fatty-diacid acylation. Research areas: Metabolic homeostasis | Neuroprotective and CNS-related pathways | Hepatic and metabolic-stress frameworks | Vascular and endothelial integrity. Storage: -20°C. For laboratory research use only.',
        ],
        8  => [ 'name' => 'TIRZEPATIDE – 10MG',
            'coa'  => [],
            'info' => 'Tirzepatide — Dual GIP/GLP-1 Receptor Agonist Research Peptide (10mg). Synthetic peptide analogue studied for its interaction with GIP and GLP-1 receptors. Molecular Formula: C225H348N48O68 | Molecular Weight: 4813.5 g/mol | CAS: 2023788-19-2. Research areas: Dual agonism of GIP and GLP-1 receptors | Receptor-binding kinetics | Metabolic-adaptation models | CNS-associated signaling. Storage: -20°C. For laboratory research use only.',
        ],
        9  => [ 'name' => 'TIRZEPATIDE – 15MG',
            'coa'  => [],
            'info' => 'Tirzepatide — Dual GIP/GLP-1 Receptor Agonist Research Peptide (15mg). Synthetic peptide analogue studied for its interaction with GIP and GLP-1 receptors. CAS: 2023788-19-2. Research areas: Dual agonism of GIP and GLP-1 receptors | Metabolic-adaptation models | Comparative incretin-analog research | Islet-cell receptor modulation. Storage: -20°C. For laboratory research use only.',
        ],
        10 => [ 'name' => 'RETATRUTIDE – 15MG',
            'coa'  => [],
            'info' => 'Retatrutide — Triple GLP-1/GIP/Glucagon Receptor Agonist Research Peptide (15mg). Synthetic multi-agonist peptide engineered to interact with GLP-1R, GIPR, and GCGR. Research areas: Incretin & glucagon biology | Energy-balance and nutrient-responsive signaling | Hepatic metabolic-pathway regulation | Islet-cell receptor modulation | Neuroendocrine metabolic-axis integration. Purity: ≥98% (HPLC-verified). Storage: -20°C. For laboratory research use only.',
        ],
        11 => [ 'name' => 'KPV – 10MG',
            'coa'  => [],
            'info' => 'KPV (Lys-Pro-Val) — Alpha-MSH C-Terminal Tripeptide Research Compound. Biologically active C-terminal tripeptide fragment of alpha-melanocyte-stimulating hormone. CAS: 135679-88-8. Research areas: Inflammatory-signaling pathway modulation | Immune-cell response profiles in in-vitro models | Peptide-mediated regulatory mechanisms at melanocortin receptor subtypes | Gut mucosal inflammation research. Purity: ≥99% (HPLC-verified). Storage: -20°C. For laboratory research use only.',
        ],
        12 => [ 'name' => 'GHK-Cu – 50MG',
            'coa'  => [],
            'info' => 'GHK-Cu — Copper Tripeptide Research Compound (50mg). Copper-binding tripeptide complex widely examined in laboratory research for its involvement in extracellular matrix (ECM) signaling, cellular repair pathways, and redox-related biological processes. Research areas: Gene-expression patterns related to tissue repair and remodeling | Cellular migration and structural adaptation within connective-tissue models | Mechanisms underlying inflammation resolution and antioxidant defense | Cutaneous regeneration pathways and ECM-driven cellular communication. Format: Lyophilized powder. For laboratory research use only.',
        ],
        13 => [ 'name' => 'TESAMORELIN – 20MG',
            'coa'  => [],
            'info' => 'Tesamorelin — GHRH Analogue for Endocrine & Metabolic Research (20mg). Synthetic growth hormone–releasing hormone (GHRH) analogue examined for its role in growth hormone (GH) axis signaling and systemic metabolic regulation. Research areas: Mechanisms governing GH-axis stimulation and downstream IGF-1 signaling | Metabolic regulation involving lipid mobilization and energy-balance pathways | Endocrine-driven changes in tissue composition | Interactions between GH signaling and neuronal or cognitive pathways. Format: Lyophilized powder. For laboratory research use only.',
        ],
        14 => [ 'name' => 'THYMOSIN ALPHA-1 – 5MG',
            'coa'  => [],
            'info' => 'Thymosin Alpha-1 (Tα1) — Immune-Modulating Research Peptide (5mg). Naturally occurring 28-amino-acid peptide originally isolated from thymic tissue. Molecular Formula: C129H215N33O55 | Molecular Weight: 3,108.4 g/mol | CAS: 62304-98-7. Research areas: CD4+/CD8+ T-lymphocyte maturation and immune-response initiation | Toll-like receptor (TLR2, TLR9) activation and downstream cytokine cascades | Dendritic cell activation and antigen presentation pathways | Pro/anti-inflammatory cytokine balance including IL-2 and IFN-γ regulation. Format: Lyophilized powder. For laboratory research use only.',
        ],
        15 => [ 'name' => 'CARTALAX – 20MG',
            'coa'  => [],
            'info' => 'Cartalax — Research-Grade Synthetic Peptide (20mg). Synthetic peptide studied in controlled laboratory settings for connective tissue and cartilage-related signaling pathways. Research areas: Connective tissue remodeling pathways | Cartilage and ECM signaling frameworks | Cellular adaptation models in musculoskeletal research | Peptide-mediated regulatory mechanisms in tissue repair. Purity: ≥99% (HPLC-verified). Storage: -20°C. For laboratory research use only.',
        ],
    ];
}

/**
 * Run the import — writes _msv_coa_images and _msv_additional_info
 * into WP post meta for each product. Returns array of result strings.
 */
function msvrouter_run_tab_import(): array {
    $results = [];
    foreach ( msvrouter_get_tab_data() as $product_id => $data ) {
        ...
        update_post_meta( $product_id, '_msv_coa_images',      wp_json_encode( $data['coa'] ) );
        update_post_meta( $product_id, '_msv_additional_info', $data['info'] );
        $n = count( $data['coa'] );
 ...
 */
function msvrouter_get_new_products(): array {
    return [
        'thymosin-alpha-1-10mg' => [
            ...
            'sku'               => 'VK-TA1-10MG',
            'image_url'         => 'https://mysecretvitality.com/wp-content/uploads/2026/05/Thymosin-Alpha-1-10mg-Peptide-450x675.png',
            'coa_purity_pdf'    => 'https://mysecretvitality.com/wp-content/uploads/2026/05/VP-1947064-P-Thymosin-Alpha-1-purity.pdf',
            'coa_endotoxin_pdf' => 'https://mysecretvitality.com/wp-content/uploads/2026/05/VP-1947064-E-Thymosin-Alpha-1-endotoxin.pdf',
        ],
        'hexarelin-10mg' => [
            'name'              => 'HEXARELIN – 10MG',
            'price'             => '120.00',
            ...
            'sku'               => 'VK-HEX-10MG',
            'image_url'         => 'https://mysecretvitality.com/wp-content/uploads/2026/05/Hexarelin-10mg-Peptide-450x675.png',
            'coa_purity_pdf'    => 'https://mysecretvitality.com/wp-content/uploads/2026/05/VP-7132129-P-Hexarelin-Purity-1.pdf',
            'coa_endotoxin_pdf' => 'https://mysecretvitality.com/wp-content/uploads/2026/05/VP-7132129-E-Hexarelin-Endotoxin.pdf',
        ],
        'oxytocin-5mg' => [
            'name'              => 'OXYTOCIN – 5MG',
            'price'             => '60.00',
            ...
            'sku'               => 'VK-OXY-5MG',
            'image_url'         => 'https://mysecretvitality.com/wp-content/uploads/2026/05/Oxytocin-5mg-Peptide-450x675.png',
            'coa_purity_pdf'    => 'https://mysecretvitality.com/wp-content/uploads/2026/05/VP-4275312-P-Oxytocin-Purity-1.pdf',
            'coa_endotoxin_pdf' => 'https://mysecretvitality.com/wp-content/uploads/2026/05/VP-4275312-E-Oxytocin-Endotoxin.pdf',
        ],
        'kisspeptin-10-10mg' => [
            ...
            'sku'               => 'VK-KISS-10MG',
            'image_url'         => 'https://mysecretvitality.com/wp-content/uploads/2026/05/Kisspeptin-10-10mg-Peptide-450x675.png',
            'coa_purity_pdf'    => 'https://mysecretvitality.com/wp-content/uploads/2026/05/VP-8975198-P-Kisspeptin-10-Purity-1.pdf',
            'coa_endotoxin_pdf' => 'https://mysecretvitality.com/wp-content/uploads/2026/05/VP-8975198-E-Kisspeptin-10-Endotoxin-1.pdf',
        ],
        'aod-9604-5mg' => [
            'name'              => 'AOD-9604 – 5MG',
            'price'             => '65.00',
            ...
            'sku'               => 'VK-AOD-5MG',
            'image_url'         => 'https://mysecretvitality.com/wp-content/uploads/2026/05/AOD-9604-5mg-Peptide-450x675.png',
            'coa_purity_pdf'    => 'https://mysecretvitality.com/wp-content/uploads/2026/05/VP-3713888-P-AOD-9604-Purity-1.pdf',
            'coa_endotoxin_pdf' => 'https://mysecretvitality.com/wp-content/uploads/2026/05/VP-3713888-E-AOD-9604-Endotoxin.pdf',
        ],
        'melanotan-2-10mg' => [
            ...
            'sku'               => 'VK-MT2-10MG',
            'image_url'         => 'https://mysecretvitality.com/wp-content/uploads/2026/05/Melanotan-2-10mg-Peptide-450x675.png',
            'coa_purity_pdf'    => 'https://mysecretvitality.com/wp-content/uploads/2026/05/VP-7423137-P-Melanotan-2-MT-2-purity.pdf',
            'coa_endotoxin_pdf' => 'https://mysecretvitality.com/wp-content/uploads/2026/05/VP-7423137-E-Melanotan-2-MT-2-endotoxin.pdf',
        ],

        // ── JEROD JUNE 2026 BATCH ─────────────────────────────────────────────
        'bpc-157-tb-500-10mg' => [
            'name'              => 'BPC-157/TB-500 – 10MG',
            'price'             => '85.00',
            'short_description' => 'BPC-157 (5mg) + TB-500 (5mg) dual-peptide research blend. 10mg lyophilized vial. ≥99% purity (HPLC). Made in USA. For laboratory research use only.',
            'description'       => 'BPC-157/TB-500 is a dual-peptide research blend combining two widely studied repair-associated peptides. Total 10mg per vial in lyophilized form.<br><br>BPC-157 (5mg) — gastric-derived pentadecapeptide studied for angiogenic and microvascular signaling, collagen organization and ECM stability, inflammatory-pathway modulation and cytokine balance, epithelial integrity and nitric-oxide–related signaling.<br><br>TB-500 (5mg) — synthetic fragment of thymosin beta-4, examined for actin-mediated cell migration and cytoskeletal organization, soft-tissue signaling and structural recovery, ECM remodeling and tissue-repair dynamics.<br><br>CAS BPC-157: 137525-51-0 | CAS TB-500: 885340-08-9. Purity: ≥99% (HPLC-verified). Storage: -20°C. Format: Lyophilized powder. For laboratory research use only.',
            'sku'               => 'MSV-BPC-TB-10MG',
            'image_url'         => 'https://mysecretvitality.com/wp-content/uploads/2026/05/Peptide-Vial-450x675.png',
            'coa_purity_pdf'    => '',
            'coa_endotoxin_pdf' => '',
        ],
        'semaglutide-5mg' => [
            ...
            'price'             => '120.00',
            'short_description' => 'Semaglutide — long-acting GLP-1 receptor agonist research peptide. 5mg lyophilized vial. ≥99% purity (HPLC). Made in USA. For laboratory research use only.',
            'description'       => 'Semaglutide is a synthetic GLP-1 receptor agonist examined in laboratory research for its role in metabolic signaling, neuroendocrine pathways, and cellular energy regulation.<br><br>Structural characteristics: High GLP-1R affinity | Resistance to DPP-4 degradation | Extended stability via C-18 fatty-diacid acylation | Engagement of PI3K/Akt-related pathways | CNS-associated receptor activity including hypothalamic signaling.<br><br>Research areas: Metabolic homeostasis | Neuroprotective and CNS-related pathways | Hepatic and metabolic-stress frameworks | Vascular and endothelial integrity.<br><br>CAS: 910463-68-2. Purity: ≥99% (HPLC-verified). Storage: -20°C. Format: Lyophilized powder. For laboratory research use only.',
            'sku'               => 'MSV-SEM-5MG',
            'image_url'         => 'https://mysecretvitality.com/wp-content/uploads/2026/05/Peptide-Vial-450x675.png',
            'coa_purity_pdf'    => '',
            'coa_endotoxin_pdf' => '',
        ...
        'tirzepatide-10mg' => [
            ...
            'price'             => '165.00',
            'short_description' => 'Tirzepatide — dual GIP/GLP-1 receptor agonist research peptide. 10mg lyophilized vial. ≥99% purity (HPLC). Made in USA. For laboratory research use only.',
            'description'       => 'Tirzepatide is a synthetic peptide analogue studied for its interaction with GIP and GLP-1 receptors in controlled research settings.<br><br>Molecular Formula: C225H348N48O68 | Molecular Weight: 4813.5 g/mol | CAS: 2023788-19-2.<br><br>Research areas: Dual agonism of GIP and GLP-1 receptors | Receptor-binding kinetics | Metabolic-adaptation models | CNS-associated signaling | Comparative incretin-analog research.<br><br>Purity: ≥99% (HPLC-verified). Storage: -20°C. Format: Lyophilized powder. For laboratory research use only.',
            'sku'               => 'MSV-TRZ-10MG',
            'image_url'         => 'https://mysecretvitality.com/wp-content/uploads/2026/05/Peptide-Vial-450x675.png',
            'coa_purity_pdf'    => '',
            'coa_endotoxin_pdf' => '',
        ...
        'tirzepatide-15mg' => [
            ...
            'price'             => '225.00',
            'short_description' => 'Tirzepatide — dual GIP/GLP-1 receptor agonist research peptide. 15mg lyophilized vial. ≥99% purity (HPLC). Made in USA. For laboratory research use only.',
            'description'       => 'Tirzepatide is a synthetic peptide analogue studied for its interaction with GIP and GLP-1 receptors. 15mg high-dose format for extended metabolic research protocols.<br><br>CAS: 2023788-19-2. Research areas: Dual agonism of GIP and GLP-1 receptors | Metabolic-adaptation models | Islet-cell receptor modulation | Neuroendocrine metabolic-axis signaling.<br><br>Purity: ≥99% (HPLC-verified). Storage: -20°C. Format: Lyophilized powder. For laboratory research use only.',
            'sku'               => 'MSV-TRZ-15MG',
            'image_url'         => 'https://mysecretvitality.com/wp-content/uploads/2026/05/Peptide-Vial-450x675.png',
            'coa_purity_pdf'    => '',
            'coa_endotoxin_pdf' => '',
        ...
        'retatrutide-15mg' => [
            ...
            'price'             => '245.00',
            'short_description' => 'Retatrutide — triple GLP-1/GIP/glucagon receptor agonist research peptide. 15mg lyophilized vial. ≥99% purity (HPLC). Made in USA. For laboratory research use only.',
            'description'       => 'Retatrutide (LY-3437943) is a synthetic multi-agonist peptide engineered to interact with GLP-1, GIP, and glucagon receptor pathways. 15mg format.<br><br>Receptor targets: GLP-1R | GIPR | GCGR.<br><br>Research areas: Incretin & glucagon biology | Energy-balance and nutrient-responsive signaling | Hepatic metabolic-pathway regulation | Islet-cell receptor modulation | Neuroendocrine metabolic-axis integration.<br><br>CAS: 2381089-83-2. Purity: ≥99% (HPLC-verified). Storage: -20°C. Format: Lyophilized powder. For laboratory research use only.',
            'sku'               => 'MSV-RET-15MG',
            'image_url'         => 'https://mysecretvitality.com/wp-content/uploads/2026/05/Peptide-Vial-450x675.png',
            'coa_purity_pdf'    => '',
            'coa_endotoxin_pdf' => '',
        ...
        'kpv-10mg' => [
            'name'              => 'KPV – 10MG',
            'price'             => '75.00',
            'short_description' => 'KPV (Lys-Pro-Val) — alpha-MSH C-terminal tripeptide research compound. 10mg lyophilized vial. ≥99% purity (HPLC). Made in USA. For laboratory research use only.',
            'description'       => 'KPV is the biologically active C-terminal tripeptide of alpha-melanocyte-stimulating hormone (α-MSH). This short tripeptide fragment retains significant bioactivity in inflammatory and immune signaling models.<br><br>Research areas: Inflammatory-signaling pathway modulation | Immune-cell response profiles in in-vitro models | Peptide-mediated regulatory mechanisms at melanocortin receptor subtypes | Gut mucosal inflammation research frameworks.<br><br>CAS: 135679-88-8 | Formula: Lys-Pro-Val | Molecular Weight: ~342 g/mol. Purity: ≥99% (HPLC-verified). Storage: -20°C. Format: Lyophilized powder. For laboratory research use only.',
            'sku'               => 'MSV-KPV-10MG',
            'image_url'         => 'https://mysecretvitality.com/wp-content/uploads/2026/05/Peptide-Vial-450x675.png',
            'coa_purity_pdf'    => '',
            'coa_endotoxin_pdf' => '',
        ...
        'ghk-cu-50mg' => [
            'name'              => 'GHK-Cu – 50MG',
            'price'             => '95.00',
            'short_description' => 'GHK-Cu — copper tripeptide research compound. 50mg lyophilized vial. ≥99% purity (HPLC). Made in USA. For laboratory research use only.',
            'description'       => 'GHK-Cu is a copper-binding tripeptide complex widely examined in laboratory research for its involvement in extracellular matrix (ECM) signaling, cellular repair pathways, and redox-related biological processes. 50mg format.<br><br>Research areas: Gene-expression patterns related to tissue repair and remodeling | Cellular migration and structural adaptation within connective-tissue models | Mechanisms underlying inflammation resolution and antioxidant defense | Cutaneous regeneration pathways and ECM-driven cellular communication.<br><br>CAS: 89030-95-5. Purity: ≥99% (HPLC-verified). Storage: -20°C. Format: Lyophilized powder. For laboratory research use only.',
            'sku'               => 'MSV-GHK-50MG',
            'image_url'         => 'https://mysecretvitality.com/wp-content/uploads/2026/05/Peptide-Vial-450x675.png',
            'coa_purity_pdf'    => '',
            'coa_endotoxin_pdf' => '',
        ...
        'tesamorelin-20mg' => [
            ...
            'price'             => '255.00',
            'short_description' => 'Tesamorelin — synthetic GHRH analogue for GH-axis research. 20mg lyophilized vial. ≥99% purity (HPLC). Made in USA. For laboratory research use only.',
            'description'       => 'Tesamorelin is a synthetic growth hormone–releasing hormone (GHRH) analogue widely examined in laboratory research for its role in growth hormone (GH) axis signaling and systemic metabolic regulation. 20mg high-dose format.<br><br>Research areas: Mechanisms governing GH-axis stimulation and downstream IGF-1 signaling | Metabolic regulation involving lipid mobilization and energy-balance pathways | Endocrine-driven changes in tissue composition | Interactions between GH signaling and neuronal or cognitive pathways.<br><br>CAS: 901758-09-6. Purity: ≥99% (HPLC-verified). Storage: -20°C. Format: Lyophilized powder. For laboratory research use only.',
            'sku'               => 'MSV-TES-20MG',
            'image_url'         => 'https://mysecretvitality.com/wp-content/uploads/2026/05/Peptide-Vial-450x675.png',
            'coa_purity_pdf'    => '',
            'coa_endotoxin_pdf' => '',
        ...
        'thymosin-alpha-1-5mg' => [
            ...
            'price'             => '105.00',
            'short_description' => 'Thymosin Alpha-1 — 28-amino-acid immune-modulating research peptide. 5mg lyophilized vial. ≥99% purity (HPLC). Made in USA. For laboratory research use only.',
            'description'       => 'Thymosin Alpha-1 (Tα1) is a naturally occurring 28-amino-acid peptide originally isolated from thymic tissue, studied for its role in immune regulation and T-cell maturation. 5mg format.<br><br>Research areas: CD4+/CD8+ T-lymphocyte maturation and immune-response initiation | Toll-like receptor (TLR2, TLR9) activation and downstream cytokine cascades | Dendritic cell activation and antigen presentation pathways | Pro/anti-inflammatory cytokine balance including IL-2 and IFN-γ regulation.<br><br>Molecular Formula: C129H215N33O55 | Molecular Weight: 3,108.4 g/mol | CAS: 62304-98-7. Purity: ≥99% (HPLC-verified). Storage: -20°C. Format: Lyophilized powder. For laboratory research use only.',
            'sku'               => 'MSV-TA1-5MG',
            'image_url'         => 'https://mysecretvitality.com/wp-content/uploads/2026/05/Peptide-Vial-450x675.png',
            'coa_purity_pdf'    => '',
            'coa_endotoxin_pdf' => '',
        ...
        'cartalax-20mg' => [
            'name'              => 'CARTALAX – 20MG',
            'price'             => '110.00',
            'short_description' => 'Cartalax — synthetic research peptide for connective tissue studies. 20mg lyophilized vial. ≥99% purity (HPLC). Made in USA. For laboratory research use only.',
            'description'       => 'Cartalax is a synthetic research peptide studied in controlled laboratory settings for its role in connective tissue and cartilage-related signaling pathways.<br><br>Research areas: Connective tissue remodeling pathways | Cartilage and ECM signaling frameworks | Cellular adaptation models in musculoskeletal research | Peptide-mediated regulatory mechanisms in tissue repair.<br><br>Purity: ≥99% (HPLC-verified). Storage: -20°C. Format: Lyophilized powder. For laboratory research use only.',
            'sku'               => 'MSV-CAR-20MG',
            'image_url'         => 'https://mysecretvitality.com/wp-content/uploads/2026/05/Peptide-Vial-450x675.png',
            'coa_purity_pdf'    => '',
            'coa_endotoxin_pdf' => '',
 ...
 * - Stores _msv_coa_purity_pdf and _msv_coa_endotoxin_pdf meta.
 * - Sets the product image via WP attachment sideloading.
 */
function msvrouter_bulk_create_products(): array {
    ...
    $results  = [];
    $products = msvrouter_get_new_products();
    ...
    foreach ( $products as $slug => $data ) {
        ...
        if ( $slug === 'melanotan-2-10mg' && ! $existing_id ) {
            ...
            if ( $mt1_id ) {
                ...
                update_post_meta( $mt1_id, '_msv_coa_purity_pdf',    $data['coa_purity_pdf'] );
                update_post_meta( $mt1_id, '_msv_coa_endotoxin_pdf', $data['coa_endotoxin_pdf'] );
                msvrouter_sideload_product_image( $mt1_id, $data['image_url'], $data['name'] );
        ...
        // Save COA PDF meta
        update_post_meta( $product_id, '_msv_coa_purity_pdf',    $data['coa_purity_pdf'] );
        update_post_meta( $product_id, '_msv_coa_endotoxin_pdf', $data['coa_endotoxin_pdf'] );

        // Sideload product image
        msvrouter_sideload_product_image( $product_id, $data['image_url'], $data['name'] );
 ...
 */
function msvrouter_sideload_product_image( int $product_id, string $image_url, string $alt ): void {
...
// Flush rewrite rules on activation so /wp-json/msv/v1/* routes resolve
register_activation_hook( __FILE__, function () {
    flush_rewrite_rules();
});

add_action( 'plugins_loaded', function () {
    MSVROUTER_Router::init();
    MSVROUTER_Admin::init();
}, 1 );

// ─────────────────────────────────────────────────────────────────────────────
// Auth REST API  —  /wp-json/msv/v1/register|login|validate
// Registered on rest_api_init (fires during WP's init hook, after plugins_loaded)
// ─────────────────────────────────────────────────────────────────────────────

add_action( 'rest_api_init', function () {

    register_rest_route( 'msv/v1', '/register', [
        'methods'             => 'POST',
        'callback'            => 'msvrouter_auth_register',
    ...
    register_rest_route( 'msv/v1', '/login', [
        'methods'             => 'POST',
        'callback'            => 'msvrouter_auth_login',
        'permission_callback' => '__return_true',
        'args' => [
            'email'    => [ 'required' => true, 'type' => 'string', 'sanitize_callback' => 'sanitize_email' ],
            'password' => [ 'required' => true, 'type' => 'string' ],
    ...
    register_rest_route( 'msv/v1', '/validate', [
        'methods'             => 'POST',
        'callback'            => 'msvrouter_auth_validate',
        'permission_callback' => '__return_true',
        'args' => [
 ...
 */
function msvrouter_make_token( int $user_id ): string {
 ...
 */
function msvrouter_verify_token( string $token ): int {
...
function msvrouter_auth_register( WP_REST_Request $req ): WP_REST_Response {
    ...
    $token = msvrouter_make_token( $user_id );
...
function msvrouter_auth_login( WP_REST_Request $req ): WP_REST_Response {
    $email    = $req->get_param( 'email' );
    $password = $req->get_param( 'password' );
    ...
    $token = msvrouter_make_token( $result->ID );
...
function msvrouter_auth_validate( WP_REST_Request $req ): WP_REST_Response {
    $token   = $req->get_param( 'token' );
    $user_id = msvrouter_verify_token( $token );
