...
define( 'MSVPO_VERSION',    '1.0.0' );
define( 'MSVPO_PLUGIN_DIR', plugin_dir_path( __FILE__ ) );
define( 'MSVPO_PLUGIN_URL', plugin_dir_url( __FILE__ ) );

// Slug that is always pinned to the very end
define( 'MSVPO_LAST_SLUG',  'bac-water-10ml' );
...
add_action( 'admin_enqueue_scripts', function ( $hook ) {
    if ( $hook !== 'woocommerce_page_msvpo-product-order' ) return;

    // WordPress ships jQuery UI Sortable — just enqueue it
    wp_enqueue_script( 'jquery-ui-sortable' );
    wp_enqueue_style(
        'msvpo-admin',
        MSVPO_PLUGIN_URL . 'assets/admin.css',
        [],
        MSVPO_VERSION
    );
    wp_enqueue_script(
        'msvpo-admin',
        MSVPO_PLUGIN_URL . 'assets/admin.js',
        [ 'jquery', 'jquery-ui-sortable' ],
        MSVPO_VERSION,
        true
    );
    wp_localize_script( 'msvpo-admin', 'VPO', [
        'ajax_url' => admin_url( 'admin-ajax.php' ),
        'nonce'    => wp_create_nonce( 'msvpo_save_order' ),
        'last_slug' => MSVPO_LAST_SLUG,
    ] );
} );
...
add_action( 'wp_ajax_msvpo_save_order', function () {
    check_ajax_referer( 'msvpo_save_order', 'nonce' );
    ...
    foreach ( $ids as $position => $product_id ) {
        ...
        $menu_order = ( $slug === MSVPO_LAST_SLUG ) ? 9999 : $position;
        ...
        update_post_meta( $product_id, '_msvpo_sort_order', $menu_order );
        $saved++;
    }
...
register_activation_hook( __FILE__, 'msvpo_set_initial_order' );

function msvpo_set_initial_order(): void {
    $products = get_posts( [
        'post_type'      => 'product',
        'post_status'    => 'publish',
        'posts_per_page' => -1,
    ...
    foreach ( $products as $key => $id ) {
        if ( get_post_field( 'post_name', $id ) === MSVPO_LAST_SLUG ) {
            $bac_id = $id;
            unset( $products[ $key ] );
        }
    }
    ...
    foreach ( $products as $pos => $id ) {
        $menu_order = ( get_post_field( 'post_name', $id ) === MSVPO_LAST_SLUG ) ? 9999 : $pos;
        wp_update_post( [ 'ID' => $id, 'menu_order' => $menu_order ] );
        update_post_meta( $id, '_msvpo_sort_order', $menu_order );
    }
}
...
function msvpo_render_page(): void {
    ...
    $regular = [];
    foreach ( $raw as $p ) {
        if ( $p->post_name === MSVPO_LAST_SLUG ) {
            $pinned[] = $p;
        } else {
            $regular[] = $p;
        }
    ...
    <div class="wrap" id="msvpo-wrap">
        ...
        <div id="msvpo-notice" style="display:none;margin:12px 0;"></div>
        ...
        <div style="display:flex;gap:12px;align-items:center;margin-bottom:20px;">
            <button id="msvpo-save" class="button button-primary" style="font-size:14px;height:38px;padding:0 24px;">
                💾 Save Order
            </button>
            <span id="msvpo-status" style="font-size:13px;color:#888;"></span>
        </div>
        ...
        <table class="wp-list-table widefat fixed striped" style="max-width:700px;">
            ...
            <tbody id="msvpo-sortable">
                <?php foreach ( $products as $pos => $product ) :
                    ...
                    $pinned_row = ( $product->post_name === MSVPO_LAST_SLUG );
                    ...
                    <tr data-id="<?php echo esc_attr( $product->ID ); ?>"
                        ...
                        class="msvpo-row<?php echo $pinned_row ? ' msvpo-pinned' : ''; ?>"
                        ...
                        <td style="text-align:center;color:#888;font-size:13px;" class="msvpo-pos">
