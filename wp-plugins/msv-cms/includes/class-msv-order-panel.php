...
/**
 * Adds a "Pending Payments" admin page under the MSV CMS menu.
 ...
 */
class MSV_Order_Panel {

    public static function init() {
        add_action( 'admin_menu', [ __CLASS__, 'register_page' ], 20 );
        add_action( 'admin_post_msv_mark_paid', [ __CLASS__, 'handle_mark_paid' ] );
        add_action( 'admin_notices', [ __CLASS__, 'show_notices' ] );
    }

    public static function register_page() {
        add_submenu_page(
            'msv-cms',                 // Parent slug (registered by MSV_Admin)
            'Pending Payments',
            'Pending Payments',
            'edit_shop_orders',
            'vcms-pending-orders',
                    ...
                    <tr>
                        ...
                        <td>
                            <form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" onsubmit="return confirm('Mark order #<?php echo esc_js( $order->get_order_number() ); ?> as PAID and send confirmation email?');">
                                <?php wp_nonce_field( 'msv_mark_paid_' . $order->get_id() ); ?>
                                <input type="hidden" name="action"   value="msv_mark_paid">
    ...
    public static function handle_mark_paid() {
        ...
        if ( ! $order_id || ! check_admin_referer( 'msv_mark_paid_' . $order_id ) ) {
        ...
        if ( $order ) {
            $order->update_status( 'processing', 'Payment confirmed manually via MSV CMS admin panel.' );
