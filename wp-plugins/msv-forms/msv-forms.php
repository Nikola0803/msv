<?php
/**
 * Plugin Name: My Secret Vitality — Forms
 * Description: Handles contact, newsletter, veterans, and waitlist form submissions. Saves newsletter signups to DB and emails no-reply@mysecretvitality.com.
 * Version: 1.3.0
 */
...
// ── Email config ──────────────────────────────────────────────────────────────
if (!defined('MSV_FORMS_TO'))          define('MSV_FORMS_TO',          'support@mysecretvitality.com');
if (!defined('MSV_FORMS_FROM'))        define('MSV_FORMS_FROM',        'no-reply@mysecretvitality.com');
if (!defined('MSV_FORMS_FROM_NAME'))   define('MSV_FORMS_FROM_NAME',   'My Secret Vitality');

if (!defined('MSV_FORMS_USE_SMTP'))    define('MSV_FORMS_USE_SMTP',    false);
if (!defined('MSV_FORMS_SMTP_HOST'))   define('MSV_FORMS_SMTP_HOST',   'mail.mysecretvitality.com');
if (!defined('MSV_FORMS_SMTP_PORT'))   define('MSV_FORMS_SMTP_PORT',   587);
if (!defined('MSV_FORMS_SMTP_SECURE')) define('MSV_FORMS_SMTP_SECURE', 'tls');
if (!defined('MSV_FORMS_SMTP_USER'))   define('MSV_FORMS_SMTP_USER',   'no-reply@mysecretvitality.com');
if (!defined('MSV_FORMS_SMTP_PASS'))   define('MSV_FORMS_SMTP_PASS',   '');

// ── DB table name ─────────────────────────────────────────────────────────────
if (!defined('MSV_FORMS_NL_TABLE'))    define('MSV_FORMS_NL_TABLE', 'msv_newsletter');

// ── Install: create table on plugin activation ────────────────────────────────
register_activation_hook(__FILE__, 'msv_install');

function msv_install() {
    global $wpdb;
    $table      = $wpdb->prefix . MSV_FORMS_NL_TABLE;
...
function msv_nl_admin_page() {
    global $wpdb;
    $table = $wpdb->prefix . MSV_FORMS_NL_TABLE;

    // CSV export
    if (isset($_GET['msv_export']) && $_GET['msv_export'] === 'csv') {
        check_admin_referer('msv_export_csv');
    ...
    $export_url = wp_nonce_url(
        admin_url('admin.php?page=msv-newsletter&msv_export=csv'),
        'msv_export_csv'
    );
    ?>
    <div class="wrap">
...
add_action('rest_api_init', function () {
    ...
    register_rest_route('msv-forms/v1', '/contact', [
        'methods'             => 'POST',
        'callback'            => 'msv_handle_contact',
        'permission_callback' => '__return_true',
    ]);

    register_rest_route('msv-forms/v1', '/newsletter', [
        'methods'             => 'POST',
        'callback'            => 'msv_handle_newsletter',
        'permission_callback' => '__return_true',
    ]);

    register_rest_route('msv-forms/v1', '/veterans', [
        'methods'             => 'POST',
        'callback'            => 'msv_handle_veterans',
        'permission_callback' => '__return_true',
    ]);

    register_rest_route('msv-forms/v1', '/waitlist', [
        'methods'             => 'POST',
        'callback'            => 'msv_handle_waitlist',
        'permission_callback' => '__return_true',
    ]);
});
...
add_action('rest_api_init', function () {
    ...
    add_filter('rest_pre_serve_request', function ($value) {
        ...
        $allowed = ['https://mysecretvitality.com', 'https://www.mysecretvitality.com'];
...
add_action('init', function () {
    if ($_SERVER['REQUEST_METHOD'] === 'OPTIONS' &&
        strpos($_SERVER['REQUEST_URI'], '/wp-json/msv-forms/') !== false) {
...
// ── Force FROM address ────────────────────────────────────────────────────────
add_filter('wp_mail_from',      function () { return MSV_FORMS_FROM; });
add_filter('wp_mail_from_name', function () { return MSV_FORMS_FROM_NAME; });

// ── Helpers ───────────────────────────────────────────────────────────────────
if (!function_exists('msv_get_post_data')) {
    function msv_get_post_data(WP_REST_Request $req) {
...
if (!function_exists('msv_send')) {
    function msv_send($subject, $body, $reply_to = '') {
        $headers = [
            'Content-Type: text/html; charset=UTF-8',
            'From: ' . MSV_FORMS_FROM_NAME . ' <' . MSV_FORMS_FROM . '>',
        ];
        if ($reply_to && is_email($reply_to)) {
            $headers[] = 'Reply-To: ' . $reply_to;
        }
        return wp_mail(MSV_FORMS_TO, $subject, msv_email_wrap($subject, nl2br(esc_html($body))), $headers);
    }
}

if (!function_exists('msv_email_wrap')) {
    function msv_email_wrap($title, $content) {
        $from_name  = MSV_FORMS_FROM_NAME;
        $from_email = MSV_FORMS_FROM;
        return <<<HTML
<!DOCTYPE html>
<html lang="en">
<head>
...
if (!function_exists('msv_ok')) {
    function msv_ok() {
        return new WP_REST_Response(['success' => true], 200);
    }
}

if (!function_exists('msv_err')) {
    function msv_err($msg, $code = 400) {
...
if (!function_exists('msv_handle_contact')) {
    function msv_handle_contact(WP_REST_Request $req) {
        $d = msv_get_post_data($req);
        ...
        if (!$name || !$email || !$message) {
            return msv_err('Missing required fields.');
        }
        ...
        msv_send("Contact: $subject — $name", $body, $email);
        return msv_ok();
    }
}

if (!function_exists('msv_handle_newsletter')) {
    function msv_handle_newsletter(WP_REST_Request $req) {
        global $wpdb;
        $d     = msv_get_post_data($req);
        $email = sanitize_email(isset($d['email']) ? $d['email'] : '');

        if (!$email) return msv_err('Email required.');
        ...
        msv_send("Newsletter Signup: $email", $body);
        return msv_ok();
    }
}

if (!function_exists('msv_handle_veterans')) {
    function msv_handle_veterans(WP_REST_Request $req) {
        $d = msv_get_post_data($req);
        ...
        if (!$name || !$email) return msv_err('Missing required fields.');
        ...
        msv_send("Veterans Discount: $name", $body, $email);
        return msv_ok();
    }
}

if (!function_exists('msv_handle_waitlist')) {
    function msv_handle_waitlist(WP_REST_Request $req) {
        $d = msv_get_post_data($req);
        ...
        msv_send("Waitlist: $product — $email", $body);
        return msv_ok();
    }
}
